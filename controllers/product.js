...
exports.create = (req, res) => {
  ...
  (form.keepExtensions = true), //file we get, extension we have
    form.parse(req, (err, fields, files) => {
      if (err) {
        return res.status(400).json({
          //send json response that something isnt ok
      ...
      if (files.photo) {
        //we send the photo from the client side (postman)
...
exports.listBySearch = (req, res) => {
  ...
  let findArgs = {};
    ...
    if (req.body.filters.length> 0) {
      query.name = { $regex: req.body.filters, $options: "i" };
...
// var itemFilterArray = req.body.filters.split(" ");

// for (let key in itemFilterArray) {
